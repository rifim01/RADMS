...
function AirportIllustration() {
  return (
    <svg viewBox="0 0 520 380" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {[0,1,2,3,4,5].map(i => (<rect key={i} x="258" y={278+i*14} width="4" height="8" rx="1" fill="white" opacity="0.7"/>))}
      {[0,1,2,3,4].map(i => (<circle key={`l${i}`} cx={168} cy={278+i*18} r="3" fill="#CC0000" opacity="0.7"/>))}
      {[0,1,2,3,4].map(i => (<circle key={`r${i}`} cx={352} cy={278+i*18} r="3" fill="#CC0000" opacity="0.7"/>))}
      ...
      {[65,90,115,140,165,190,215,240,265,290,315].map((x,i) => (<rect key={i} x={x} y={162} width={16} height={20} rx="2" fill="#bfdbfe" opacity="0.8"/>))}
      {[65,90,115,140,165,190,215,240,265,290,315].map((x,i) => (<rect key={i} x={x} y={196} width={16} height={20} rx="2" fill="#bfdbfe" opacity="0.6"/>))}
      <rect x="160" y="225" width="80" height="50" rx="3" fill="#f8fafc" stroke="#e2e8f0"/>
  ...
  return (
    <svg viewBox="0 0 310 92" xmlns="http://www.w3.org/2000/svg" className={className}>
      <defs>
        ...
        <filter id="lgSh"><feDropShadow dx="0" dy="3" stdDeviation="3" floodColor="rgba(0,0,0,0.3)"/></filter>
      </defs>
      ...
      <text x="44" y="64" textAnchor="middle" fontFamily="Impact,Arial Black,Arial,sans-serif" fontWeight="900" fontSize="50" fill="white" filter="url(#lgSh)" letterSpacing="-1">RI</text>
      <line x1="91" y1="10" x2="91" y2="78" stroke="#CC0000" strokeWidth="2.5" opacity="0.3"/>
      <text x="100" y="74" fontFamily="Impact,Arial Black,Arial,sans-serif" fontWeight="900" fontSize="68" fill="url(#lgFim)" filter="url(#lgSh)" letterSpacing="-1">FIM</text>
      ...
      <text x="2" y="90" fontFamily="Arial,Helvetica,sans-serif" fontWeight="600" fontSize="9" fill="#555" letterSpacing="0.6">PT. RIFIM INTERNATIONAL GEMILANG</text>
    </svg>
  )
}
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      <div className="hidden lg:flex lg:w-[55%] flex-col bg-white border-r border-slate-200 relative overflow-hidden">
        ...
        <div className="flex-1 px-8 pb-6"><AirportIllustration /></div>
        <div className="px-10 pb-8">
          <div className="flex items-center gap-8 bg-slate-50 rounded-2xl p-4 border border-slate-200">
            {[{n:'7',l:'Cabang'},{n:'300+',l:'Driver'},{n:'500+',l:'Penjemputan/Hari'}].map(s => (
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 lg:px-12 bg-white lg:bg-slate-50">
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-5 bg-red-50 border border-red-100 rounded-xl p-3">
            <p className="text-red-700 text-xs font-bold mb-2">Akun Tersedia</p>
            <div className="space-y-1">
              {DEMO_ACCOUNTS.map(acc => (
                <button key={acc.email} type="button" onClick={() => fillDemo(acc)}
                  className="w-full flex items-center justify-between bg-white hover:bg-red-50 border border-red-100 hover:border-red-300 rounded-lg px-3 py-2 transition">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} required disabled={loading}
                  ...
                  className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-300 transition"/>
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                  ...
                  className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl py-3 pl-10 pr-12 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-300 transition"/>
