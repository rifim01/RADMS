  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-20">
      <Header
        ...
        rightAction={
          <button onClick={handleRefresh} disabled={refreshing} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-700 transition-colors">
      ...
      <div className="px-4 py-4 space-y-4">
        {myQueueEntry ? (
          <div className="bg-gradient-to-br from-blue-600/25 to-indigo-600/15 border border-blue-500/40 rounded-2xl p-5">
              ...
              <div className="flex-1">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white font-bold text-lg">{estWait !== null ? `${estWait}m` : '-'}</p>
            ...
            {myQueueEntry.status === 'CALLED' && (
              <div className="mt-4 bg-blue-500/20 border border-blue-400/40 rounded-xl p-3 text-center">
                <p className="text-blue-300 font-semibold animate-pulse">Anda Dipanggil! Segera menuju zona penjemputan.</p>
                <button onClick={() => setShowValidasi(true)} className="w-full mt-3 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-colors">
          ...
          <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5 text-center">
            ...
            <p className="text-slate-500 text-sm mt-1">
              {!isOnline ? 'Aktifkan status online di Beranda' : !inGeofence ? 'Masuki area bandara untuk bergabung antrian' : 'Mendaftarkan ke antrian...'}
            </p>
          </div>
        )}
        ...
        <div className="flex items-start gap-3 bg-slate-800/40 border border-slate-700/30 rounded-xl p-3">
          ...
          <p className="text-slate-500 text-xs">Antrian otomatis didaftarkan saat Anda masuk area geofence bandara dan status online. Estimasi tunggu berdasarkan rata-rata 8 menit per kendaraan.</p>
        </div>

        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-slate-400 text-xs font-medium uppercase tracking-wider">Daftar Antrian ({activeQueue.length})</p>
          ...
          <div className="space-y-2">
            {activeQueue.map((entry) => (
              <QueueCard key={entry.id} entry={entry} isCurrentDriver={entry.driverId === driver?.id} />
            ))}
              ...
              <>
                ...
                <form onSubmit={handleValidasi} className="space-y-3">
                  ...
                  <div>
                    ...
                    <input type="text" value={staffId} onChange={e => setStaffId(e.target.value)} placeholder="Contoh: RIF0125" className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-red-500" required />
                  </div>
                  <div>
                    <label className="text-slate-400 text-xs mb-1 block">Nama Staff</label>
                    <input type="text" value={staffNama} onChange={e => setStaffNama(e.target.value)} placeholder="Nama lengkap staff" className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-red-500" required />
                  </div>
                  <button type="submit" disabled={validasiLoading} className="w-full py-3 bg-red-600 hover:bg-red-500 disabled:opacity-60 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-colors">
                    {validasiLoading ? (
